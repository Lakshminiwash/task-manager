...
const Box = () => {
  ...
  //   const data = JSON.parse(localStorage.getItem("alltask"));
  //   return data || [];   // 🔥 must fix
  // });
  const [task, setTask] = useState(() => {
    try {
      const data = JSON.parse(localStorage.getItem("alltask"));
  ...
  if (editTask !== null) {
    ...
    let oldTask = Array.isArray(task) ? [...task] : [];
    oldTask.push(val);
    setTask(oldTask);
  }
}
  ...
  const completedCount = Array.isArray(task)
